    ...
    for (std::string line; std::getline(file, line);) {
        ...
        for (int i = 0; i < line.size(); ++i) {
            ...
            if (collect_bid) {
                ...
                else if (letter == 'J') letter = '1'; // make J weakest
                else if (letter == 'T') letter = 'V';
                hand.cards[bid] = letter;
                if (letter == '1') {
                    hand.jokers++;
                } else {
                    auto& count = counts[letter];
                    ++count;
                    if (highest_count == nullptr || (*highest_count) < count) {
                        highest_count = &count;
                    }
                }
                ++bid;
        ...
        // add jokers to highest count card
        if (highest_count != nullptr) (*highest_count) += hand.jokers;
        else five = true;
        // hand
